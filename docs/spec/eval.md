...
| `'host'`   | host-value embed     | wraps an arbitrary host-side value (typed host fn, type value, `IO`, opaque JS object). Eval yields the value as-is; `print` cannot round-trip it to source. Used by `g.host`. |
