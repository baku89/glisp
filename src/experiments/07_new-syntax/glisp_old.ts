...
export type AST = number | boolean | ASTFn | ASTSymbol | ASTFncall | ASTScope
...
export interface PDGScope extends PDGBase {
	type: 'scope'
...
export type PDG = PDGFncall | PDGSymbol | PDGScope | PDGFn | PDGValue
...
function getAllDeps(pdg: PDG) {
	const dep: (PDGSymbol | PDGFn | PDGFncall | PDGScope)[] = Array.from(pdg.dep)
	if (pdg.parent) {
	...
	function traverse(pdg: PDG, env: Env): PDG {
		...
		if (pdg.type === 'fncall') {
			...
			if (!(pdg.return in pdg.values)) {
				pdg.resolved = new Error(
					`Return symbol ${pdg.return} is not defined in the scope`
				)
				return pdg
			}
...
;(async function () {
	...
	await test('(let [a (+ 1 2)] a)', 3)
	await test('(let [a b b a c (+ 1 2)] c)', 3)
	await test('(+ (let [a 10 b (* a 2)] b) 1)', 21)
	await test('(let [a a] a)', 'error')
	await test('(let [a 10 b (let [a 20] a) c (+ a b)] c)', 30)
	await test('(+ @"./1" 10)', 20)
	await test('(+ 10 (+ @"../0" 1))', 21)
	await test('(let [a (+ 1 2) b (+ @"../a/1" a)] b)', 5)
})()
